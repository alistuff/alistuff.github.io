    ...
    ns.RayCaster.prototype = {
        ...
        getEntitiesProjections: function () {
            ...
            for (var i = 0; i < length; i++) {
                ...
                var pz = scale * this.level.gridSize + scale * entity.rotz;
                var iy = this.rotz - ph + pz / 2;
        ...
        render2d: function (context) {
            ...
            context.fillRect(this.x, 0, this.map.realWidth * scale, this.map.realHeight * scale);
            ...
            //    context.beginPath();
            //    context.moveTo(this.x + step * scale, 0);
            //    context.lineTo(this.x + step * scale, this.map.realHeight * scale);
            //    context.stroke();
            //}
            ...
            //    var step = i * this.map.gridSize + 0.5;
            //    context.beginPath();
            //    context.moveTo(this.x + 0, step * scale);
            //    context.lineTo(this.x + this.map.realWidth * scale, step * scale);
            //    context.stroke();
            //}

            //draw solid grid
            context.fillStyle = 'rgba(0,199,255,0.5)';

            for (var y = 0; y < this.map.height; y++) {
                for (var x = 0; x < this.map.width; x++) {
                    if (this.map.isBlock(x, y)) {
                        context.fillRect(this.x + x * this.map.gridSize * scale, y * this.map.gridSize * scale, this.map.gridSize * scale, this.map.gridSize * scale);
                    }
                }
            }
            ...
            context.beginPath();
            context.arc(this.x + this.viewer.x * scale, this.viewer.y * scale, 6 * scale, 0, 2 * Math.PI, false);
            context.fill();

            // context.fillStyle = 'blue'; 
            for (var i = 0; i < this.map.entities.length; i++) {
                var entity = this.map.entities[i];
                context.fillStyle = entity.col;
                context.beginPath();
                context.arc(this.x + entity.x * scale, entity.y * scale, 6 * scale, 0, 2 * Math.PI, false);
                context.fill();
            }
