...
//
Texture.fromUrl = function (fileUrl) {
...
Texture.fromColor = function (width, height, color) {
...
Texture.prototype = {
            ...
            for (var j = offsetY; j < clipRect.height; j++) {
                for (var i = offsetX; i < clipRect.width; i++) {
                    ...
                    if (rgba.a == 255) {
                        ...
                        alpha = rgba.a / 255;
                        data[index] = rgba.r * alpha + (1 - alpha) * data[index];
                        data[index + 1] = rgba.g * alpha + (1 - alpha) * data[index + 1];
                        data[index + 2] = rgba.b * alpha + (1 - alpha) * data[index + 2];
                        data[index + 3] = rgba.a * alpha + (1 - alpha) * data[index + 3];
                    }
                }
            }
    ...
    //render: function (context, sx, sy, sw, sh, dx, dy, dw, dh) {
    //    throw 'not supported';
    //},
};
